...
finding number of chironomid genera identified in NCRN
```{r}
invert_genera <- read.csv("NPS_counts2.csv")
# View(invert_genera)

##filtering for chironomids
chironomids <- invert_genera %>%
  filter(FAMILY=="Chironomidae")

length(unique(chironomids$GENUS))
##74
##-1 for Cricotopus/Orthocladius = 73
...
##tolerance level of macroinvertebrate taxa
# inverts_current <- inverts %>%
#   filter(year>2018) %>%
#   select(SubjectTaxonomicName) %>%
#   distinct()
...
##tolerance level of macroinvertebrate taxa
inverts_current <- new_invert_counts2 %>%
  filter(year>2018) %>%
  group_by(Taxon) %>%
...
metadata <- read.csv("bss_macroinvert_lookup.csv") %>%
  select(Taxon, TolVal)

taxa_tolval <- merge(inverts_current, metadata, by="Taxon") %>%
  mutate(TolVal=case_when(Taxon=="Sympotthastia"~8.2,
                          TRUE~TolVal)) %>%
  mutate(tolerance=case_when(TolVal<=3~"Sensitive",
                             TolVal>3 & TolVal<7~"Intermediate",
                             TolVal>=7 | Taxon=="Cricotopus/orthocladius" ~"Tolerant",
                             TRUE~"Unclassified/unknown")) %>%
  mutate(number=1) %>%
  group_by(tolerance) %>%
  summarize(tol_taxa=sum(number))

##tolvals different from metadata
##Acerpenna (2.6), antocha (8), 

tol_counts <- taxa_tolval %>%
  group_by(tolerance) %>%
  summarize(tol_count=sum(count))
...
gwmp_fish <- fish %>%
  subset(Park=="GWMP") %>%
  subset(Site=="TURU") %>% ##plug in site here
  group_by(SubjectTaxonomicName, year) %>%
  summarize(sum_fish=sum(as.numeric(ResultMeasureValue), na.rm=T)) %>%
