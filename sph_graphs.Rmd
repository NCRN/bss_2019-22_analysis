...
cato_habitat_graph <- ggplot() +
  ...
  geom_text(data=cato_habitat[cato_habitat$value==0,], aes(x=as.factor(x.axis), y=value), label="•",
...
rocr <- sph %>%
  ...
  mutate(year2=case_when(year==2008~"year1",
                         ...
                         year==2019~"year2",
                         year==2022~"year2")) %>%
  mutate(name2=case_when(Site=="BAKE"~"Battery Kemble Run*‡",
